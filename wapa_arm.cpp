...
// Thread 0 and 28 belong to the same physical core: 1 and 29, x and x + SMTDIST
#define SMTDIST 28

// --
// 2024 - 01 - 11 -> POT - Optimal transport regularized OT -< Nueva version
void WAPA::apply(uint64_t current_interval, const tasklist_t &tasklist)
{
	...
	if( size_comb >= num_comb)
	{
		...
		for (const auto &task_ptr : tasklist) {
            ...
            double cycles       = task.stats[0].last("cycles");
            double instructions = task.stats[0].last("INST_RETIRED");
			// ----
			...
			for (const auto &Task_co_runner : tasklist) {
                ...
                if (taskID == ID_co_runner ){ M [ID_co_runner][taskID] = 100.0; } //mucho coste 
            } // End for
			    double inst = 0, cycl = 0, ipc = -1;
			    assert((stats == "total") | (stats == "interval"));
			    if (stats == "total"){ 
                    inst = task.stats[0].sum("INST_RETIRED"); cycl = task.stats[0].sum("cycles");
			    } else if (stats == "interval"){
			    	inst = task.stats[0].last("INST_RETIRED"); cycl = task.stats[0].last("cycles");
			    }
			    ipc = inst / cycl;
			    ipcTotal += ipc;
    	} 
